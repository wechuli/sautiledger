            ...
            <li key={s.trackingCode}>
              <Card>
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  {s.urgency && <Badge tone={urgencyTone(s.urgency)}>{s.urgency}</Badge>}
                  {s.category && <Badge>{s.category}</Badge>}
                  {s.mandateStatus && <Badge tone={statusTone(s.mandateStatus)}>{s.mandateStatus}</Badge>}
                </div>
                {s.mandateId && s.mandateTitle && (
                  <p className="mt-2 text-sm">
                    Mandate:{" "}
                    <Link to={`/mandates/${s.mandateId}`} className="text-primary underline">
                      {s.mandateTitle}
                    </Link>
                  </p>
                )}
                {s.targetAuthority && (
                  <p className="text-xs text-muted-foreground">
                    Routed to {s.targetAuthority.name} ({s.targetAuthority.level})
                  </p>
                )}
