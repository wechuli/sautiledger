...
import { AlertTriangle, CheckCircle2, Landmark, ListChecks } from "lucide-react";
...
export function DashboardPage() {
  ...
  const [recent, setRecent] = useState<Awaited<ReturnType<typeof api.listMandates>>["items"]>([]);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    Promise.all([api.dashboardStats(), api.listMandates({ pageSize: 6, sort: "recent" })])
      .then(([s, m]) => {
        setStats(s);
        setRecent(m.items);
      })
  ...
  return (
    <div className="space-y-6">
      <section className="rounded-lg border border-border bg-card px-5 py-6">
        <p className="text-xs font-semibold uppercase tracking-wide text-primary">SautiLedger</p>
        <h1 className="mt-1 max-w-3xl text-3xl font-bold md:text-4xl">
          Community concerns into verifiable civic mandates.
        </h1>
        <p className="mt-2 max-w-2xl text-sm text-muted-foreground">
          Anonymized submissions, aggregate dashboards, and institution response tracking — built for
          Kenya-first public participation.
        </p>
      </section>
      ...
      {stats && (
        <>
          <section className="grid gap-4 md:grid-cols-4">
            <Metric icon={<ListChecks />} label="Community Mandates" value={String(stats.totals.mandates)} />
            <Metric icon={<Landmark />} label="Authorities" value={String(stats.totals.authorities)} />
            <Metric
              icon={<CheckCircle2 />}
              label="Acknowledged"
              value={`${stats.totals.acknowledgedPct}%`}
            />
            <Metric icon={<AlertTriangle />} label="Resolved" value={`${stats.totals.resolvedPct}%`} />
          </section>

          <section className="grid gap-6 lg:grid-cols-[1.4fr_1fr]">
            <Card>
              <CardHeader title="New mandates (30 days)" subtitle="Aggregate civic trend." />
              <div className="h-64">
                <ResponsiveContainer>
                  <LineChart data={stats.trend30d}>
                    ...
                    <Tooltip />
                    <Line type="monotone" dataKey="count" stroke="#16806f" strokeWidth={2} dot={false} />
                  </LineChart>
                </ResponsiveContainer>
              </div>
            </Card>
            ...
            <Card>
              <CardHeader title="Top counties" subtitle="Where mandates are concentrated." />
              <div className="h-64">
                <ResponsiveContainer>
                  <BarChart data={stats.byCounty} layout="vertical" margin={{ left: 24 }}>
                    <CartesianGrid strokeDasharray="3 3" />
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {stats.topResponsiveness.map((row) => (
                      <tr key={row.authority.id} className="border-b border-border last:border-0">
                        <td className="py-2 font-medium">
                          {row.authority.name}
                          <span className="ml-2 text-xs text-muted-foreground">{row.authority.level}</span>
                        </td>
                        <td className="py-2">{row.assigned}</td>
                        <td className="py-2">{row.acknowledged}</td>
                        <td className="py-2">{row.resolved}</td>
                        <td className="py-2">
                          <Badge tone="primary">{row.responsivenessIndex.toFixed(2)}</Badge>
                        </td>
                      </tr>
                    ))}
                  </tbody>
      ...
      <Card>
        <CardHeader title="Recent mandates" subtitle="Aggregate public view; raw submissions stay private." />
        {recent.length === 0 ? (
